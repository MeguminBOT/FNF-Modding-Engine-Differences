...
| **Scroll speed change**  | `ScrollSpeed`                 | `Change Scroll Speed`     | `Scroll Speed Change`                |
...
| **Set health icon**      | `SetHealthIcon`           | N/A                       | N/A (scripted)                       |
...
| **Event handler** | `ScriptedSongEvent` class (HScript) | `onEvent(name, v1, v2)` callback       | `onEvent(event)` callback with `event.event.name` + `event.event.params[]` |
